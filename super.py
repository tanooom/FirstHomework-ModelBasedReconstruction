...
results_dir = Path("super_results")
...
K = operators.DownScaling(
    ...
    downscale_factor = 2, #dimezza la risoluzione
    mode = "avg", #fa average pooling
)

#Forward Problem: downascaling + rumore gaussiano
# a differenza di denoising e deblur, y_delta ha shape diversa da x_true
# questo avviene perché K riduce la risoluzione. è il cuore della super resolution
# ovvero partire da immagine con bassa risoluzione e ricostruire quella ad alta risoluzione
...
#---- CGLS - Tikhonov -----
# nel caso di super resolution è un problema fortemente mal posto, in quanto da immagine
# a bassa risoluzione si deve risalire ad una ad alta risoluzione
...
# lambda più alto rispetto al denoising perché la super resolution 
# richiede più regolarizzazione
...
save_image(normalize(x_tv), results_dir / "sr_tv.png")
...
fig.suptitle("Metriche vs Iterazioni - Super Resolution", fontsize=14)
