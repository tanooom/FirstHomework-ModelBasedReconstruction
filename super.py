...
# Due configurazioni: downscaling leggero vs pesante
configurazioni = [
    {"downscale_factor": 2, "noise_level": 0.01},  # dimezza la risoluzione, poco rumore
    {"downscale_factor": 4, "noise_level": 0.05},  # riduce a un quarto, piu rumore
]
...
for cfg in configurazioni:
    ...
    print(f"\n{'='*60}")
    print(f"CONFIGURAZIONE: downscale={downscale_factor}, noise={noise_level}")
    print(f"{'='*60}")

    # Crea cartella risultati specifica per questa configurazione
    results_dir = Path(f"super_results/ds{downscale_factor}_n{noise_level}")
    results_dir.mkdir(parents=True, exist_ok=True)

    # Operatore di downscaling
    # downscale_factor controlla di quanto viene ridotta la risoluzione
    # mode='avg' fa average pooling — simula un sensore reale che media i pixel
    ...
    # Forward problem: downscaling + rumore gaussiano
    # NOTA: y_delta ha shape diversa da x_true perche K riduce la risoluzione
    # es. con downscale_factor=2: x_true (1,1,512,512) -> y_delta (1,1,256,256)
    y       = K(x_true)
    ...
    save_image(normalize(y_delta), results_dir / "lowres_image.png")
    ...
    # TIKHONOV via CGLS
    # ════════════════════════════════════════════════════════════════════════
    # Nel problema di super resolution CGLS deve risalire da una immagine
    # a bassa risoluzione a una ad alta risoluzione — problema fortemente mal posto
    print("\n--- CGLS Tikhonov ---")
    ...
    save_image(normalize(x_cgls), results_dir / "sr_cgls.png")

    # ════════════════════════════════════════════════════════════════════════
    # TV con lambda_best (Best Metric)
    # ════════════════════════════════════════════════════════════════════════
    # lambda piu alto rispetto al denoising perche la super resolution
    # e un problema piu mal posto e richiede piu regolarizzazione
    print("\n--- TV (Best Metric) ---")
    ...
    save_image(normalize(x_tv_best), results_dir / "sr_tv_best.png")
    ...
    save_image(normalize(x_tv_dp), results_dir / "sr_tv_dp.png")
    ...
    save_image(normalize(x_tpv), results_dir / "sr_tpv.png")

    # ════════════════════════════════════════════════════════════════════════
    # PLOT METRICHE VS ITERAZIONI
    # ════════════════════════════════════════════════════════════════════════
    fig, axes = plt.subplots(3, 4, figsize=(20, 12))
    fig.suptitle(f"Metriche vs Iterazioni - Super Resolution (ds={downscale_factor}, noise={noise_level})", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig(results_dir / "metrics_superresolution.png", dpi=150, bbox_inches='tight')
    plt.show()
