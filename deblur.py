...
#Creazione del Forward problem (blurring + rumore)
A_blur = operators.Blurring(
    img_shape=(256,256),
...
noise_level = 0.02
y_blurred = A_blur(x_gt)

noise = noise_level * torch.rand_like(y_blurred)
y_noisy = y_blurred + noise

#Tikhonov
solver_cgls = solvers.CGLS(K=A_blur)
n_iterazioni = 100
x_cgls, info_cgls = solver_cgls(y_noisy, x_gt, None, maxiter=n_iterazioni)

#Total Variation con p = 1
solver_tv = solvers.ChambollePockTpVUnconstrained(K=A_blur)
lambdas = [0.01, 0.05, 0.1, 0.3, 0.5]

risultati_tv = []
for lmbda in lambdas:
    print(f"Eseguo TV con λ={lmbda}...")
    x_tv, info_tv = solver_tv(y_noisy, x_gt, None, lmbda=lmbda, maxiter=100, p=1)
...
fig1.suptitle("Deblur - Tikhonov (CGLS - Early Stopping)", fontsize=13)

axes1[0].imshow(x_gt.squeeze().cpu().detach().numpy(), cmap='gray')
axes1[0].set_title("Ground Truth")
axes1[0].axis('off')

axes1[1].imshow(y_blurred.squeeze().cpu().detach().numpy(), cmap='gray')
axes1[1].set_title("Blurred")
axes1[1].axis('off')

axes1[2].imshow(y_noisy.squeeze().cpu().detach().numpy(), cmap='gray')
axes1[2].set_title(f"Blurred + Rumore (noise={noise_level})")
axes1[2].axis('off')

axes1[3].imshow(x_cgls.squeeze().cpu().detach().numpy(), cmap='gray')
axes1[3].set_title(f"Tikhonov - CGLS (iter={n_iterazioni})")
axes1[3].axis('off')
...
fig2.suptitle("Deblur - Total Variation (p=1) - Chambolle-Pock (confronto λ)", fontsize=13)
...
# Figura 3: TpV con 5 valori di p
fig3, axes3 = plt.subplots(1, 5, figsize=(25, 5))
fig3.suptitle(f"Deblur - Total p-Variation (λ={lmbda_tpv}, confronto p)", fontsize=13)
...
# Figura 6: metriche TpV (confronto p)
...
fig7.suptitle("Grid Search TpV - Heatmap metriche finali (Deblur)", fontsize=13)
...
psnr_blurred = 10 * torch.log10(1 / torch.mean((x_gt - y_noisy)**2)).item()
print(f"{'Blurred + Rumore (baseline)':<30} {psnr_blurred:>10.4f} {'  -':>10} {'  -':>10} {'  -':>6}")
